...
public class ClsConsulta {
    ...
    public static ArrayList<String> llenarcomboseguro() {
        ArrayList<String> lista1=new ArrayList<String>();
        PreparedStatement ps=null;
        ResultSet rs=null;
        Connection con=ClsConexion.Conectar();
        String sql="SELECT * FROM tb_pension;";
        try {
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery();
            while(rs.next())
            {
                lista1.add(rs.getString("nombre_pension"));
            }
        } catch (Exception e) {
            System.err.println("Error");
        }
    ...
    public static ArrayList<String> llenarcombotasa() {
        ArrayList<String> lista=new ArrayList<String>();
        PreparedStatement ps=null;
        ResultSet rs=null;
        Connection con=ClsConexion.Conectar();
        String sql="SELECT * FROM tb_tasa;";
        try {
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery();
            while(rs.next())
            {
                lista.add(rs.getString("nombre_tasa"));
            }
        } catch (Exception e) {
            System.err.println("Error");
        }
