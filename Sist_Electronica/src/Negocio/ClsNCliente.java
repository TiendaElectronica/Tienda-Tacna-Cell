...
public class ClsNCliente {
    ...
    public boolean MtdModificarcliente(ClsECliente objEC) {
        try {
            String SQL="UPDATE tb_cliente set dni=?,edad=?,nombre=?,direccion=?,email=?,telefono=?,observacion=?  where id_cliente=? ";
            Connection con=ClsConexion.Conectar();
    ...
    public void MtdRegistrarCliente(ClsECliente objEC) {
        try{
            String SQL="insert into tb_cliente(dni,edad,nombre,direccion,email,telefono,observacion) values (?,?,?,?,?,?,?) ";
            Connection con=ClsConexion.Conectar();
